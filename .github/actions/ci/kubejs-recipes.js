...
const pattern_totals =
  /Added (\d+) recipes, removed (\d+) recipes, modified (\d+) recipes, with (\d+) failed recipes and (\d+) fall-backed recipes/;
...
fsp.readFile('logs/kubejs/server.txt').then(data => {
  ...
  for (const [group, recipes] of Object.entries(groups)) {
    fsp.writeFile(
      `kubejs/exported/recipes/${group}.json`,
  ...
  console.log(
    `::notice::${
      Object.entries(groups['kjs']).length
    } KubeJS recipes with kjs_, ${
      Object.entries(groups['md5']).length
    } recipes with md5_, and ${
      Object.entries(groups['duplicate']).length
    } duplicates.`
  );
});
