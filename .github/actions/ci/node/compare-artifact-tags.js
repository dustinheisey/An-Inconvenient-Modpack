...
const kubejs = {
    [a]: JSON.parse(
        fs.readFileSync(`${a}/kubejs/exported/kubejs-server-export.json`)
    ),
    [b]: JSON.parse(
...
types.forEach((type) => {
    [a, b].forEach((c) => {
        ...
        fs.writeFileSync(
            `${c}/kubejs/exported/tags/${type}.txt`,
            lines.join("\n")
        );
    });

    let diff = execSync(
        `git --no-pager diff --color --no-index '${a}/kubejs/exported/tags/${type}.txt' '${b}/kubejs/exported/tags/${type}.txt' || true`
    ).toString();
    ...
    diff.split("\n").forEach((line) => {
        ...
        if (line == "\x1B[31m-\x1B[m" || line == "\x1B[32m+\x1B[m")
            line = "\x1B[m";
